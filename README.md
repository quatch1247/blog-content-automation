...
- **개선 포인트:**
  ...
  - 각 태스크별 **상태 추적(Progress Tracking)** 및 **실패 시 재시도(Retry)** 지원  
...
- **로컬 모델 전환**  
  - OpenAI API에 전적으로 의존하지 않도록,  
    추후 **LLaMA / Mistral / Gemma** 등 로컬 경량 모델을 활용  
...
- **(고난도) 파인튜닝(Fine-tuning)**  
  - 기존 PDF 데이터셋(요약문, 메타데이터 등)을 학습시켜  
    모델이 “블로그 콘텐츠 문체와 구조”에 특화되도록 미세 조정  
  - 단, **비용·데이터 관리·성능 검증의 복잡성**이 높음  
    MVP 단계에서는 RAG 방식으로 확장성을 확보하고, 추후 서비스가 안정화되면 파인튜닝 및 로컬 모델로의 전환을 검토하는 것이 이상적.
...
> 이 과정은 “AI 작성 보조 도구”로서의 서비스 신뢰성을 높이는 핵심입니다.

---

### 3-2. 자잘한 오류 개선 및 URL 할루시네이션 대응

잘못된 출처(URL) 또는 존재하지 않는 인용문을 생성하는 **할루시네이션 문제**를 줄이기 위해  
**출처 검증 모듈** 또는 **URL 유효성 검사 로직**을 포함한 후처리 파이프라인을 추가할 수 있습니다.

- 예시: `requests.head(url)`로 실제 응답 코드 확인  
  → 존재하지 않으면 해당 출처/URL을 결과에서 제외 처리

---

### 3-3. 이미지 활용의 방향성과 한계

현재 PDF에서 이미지 파일을 수집하고 있지만, 이를 본문 문맥에 직접 활용하지 않는 이유는 다음과 같습니다.
...
**이미지-텍스트 연관성 판단 모델**(예: CLIP 기반)을 통해  
의미적 유사도를 측정하고,  
**자동 캡션 생성** 또는 **연관 이미지 추천 기능**으로 확장할 수 있습니다.

---

### 3-4. PDF 기반 수집의 한계와 HTML 크롤링의 대안

현재는 **PDF를 파싱하여 텍스트를 추출**하고 있으나,  
사실상 **원본 블로그를 직접 크롤링**하는 것이  
정보 풍부도·정밀도 측면에서 훨씬 유리합니다.

#### 예시: 네이버 블로그 원문 크롤링 프로세스

1. 네이버 API로 검색 키워드에 해당하는 블로그 URL 목록 확보
2. Selenium 등으로 각 URL을 열고 **HTML 문서 수집**
3. BeautifulSoup으로 HTML 파싱 → `title`, `contents`, `postdate` 등 추출
4. 결과를 CSV 등 파일로 저장

이 방식은 **PDF 파싱보다 훨씬 풍부한 텍스트, 태그, 메타데이터** 확보 가능  
향후 RAG(Retrieval-Augmented Generation)나 **메타데이터 기반 추천 시스템**에도 유리
